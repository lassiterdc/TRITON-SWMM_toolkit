...
main_scratch: /home/dcl3nd/dev/agentic-workspace/library/_scratch/workspaces/TRITON-SWMM_toolkit_05-15_0904_per-sub-analysis-system-configs-pwi.md
last_sync_hashes:
  '### Phase Audit History': 9b2f44c6a33d7f391f2e54c09b4cbfb32493908ec415f76dfcce3adac935fc43
  '# Follow-up Ideas': d3295ae0bc1167b7dc9bb443fe9aedf610104da0bc81d7185a3f601bb0b2b4fa
harness: claude-code
next_phase: '3'
next_phase_title: Snakemake workflow generation
next_phase_doc: /home/dcl3nd/dev/agentic-workspace/library/docs/planning/projects/TRITON-SWMM_toolkit/features/per_sub_analysis_system_configs/3 snakemake workflow generation.md
leave_off_step: Phase 2 complete and committed; entering Phase 3 (Snakemake workflow generation)
worktree_path: /home/dcl3nd/dev/TRITON-SWMM_toolkit/.claude/worktrees/toolkit_05-15_0904_per-sub-analysis-system-configs-pwi
created: '2026-05-15T17:59:09'
smoke_test: false
sidecar_kind: in_flight_multiphase
baseline_writable_content:
  '### Phase Audit History': "\n\n\n%%\n<!-- TPL-INSTRUCTION: Populated after each phase completes. Append one row per phase to the table below. The QAQC audit-on-diff gate uses phase-local diff (files changed since previous phase commit), not branch-cumulative. -->\n\n| Phase | Phase-Local Diff Files | Audit-on-Diff Violations | Resolution |\n|-------|------------------------|--------------------------|------------|\n| 1 | `src/TRITON_SWMM_toolkit/sensitivity_analysis.py` (+132/-3); `tests/test_PC_07_per_sa_system_configs.py` (new) | 0 (clean — exit 0, no violations) | N/A — gate cleared. Tests: `tests/test_PC_07_per_sa_system_configs.py` 6/6 pass; `tests/test_synth_05_sensitivity_analysis_with_snakemake.py` fast tier 4/4 pass as backward-compat proxy for PC_05 (real Norfolk data unavailable in this worktree). |\n| 2 | `src/TRITON_SWMM_toolkit/sensitivity_analysis.py` (+40/-4); `tests/test_PC_07_per_sa_system_configs.py` (+58/-0) | 0 (clean — exit 0, no violations) | N/A — gate cleared. Tests: `tests/test_PC_07_per_sa_system_configs.py` 8/8 pass (incl. 2 new Phase 2 tests); `test_synth_05_sensitivity_analysis_with_snakemake.py` fast 4/4 pass (PC_05 proxy); `test_synth_04_multisim_with_snakemake.py` fast 11 passed + 1 slow-skipped (PC_04 proxy). Real-data PC_04/PC_05 unavailable in worktree. |\n%%\n\n"
...
- Leave-off step: Phase 2 complete and committed; entering Phase 3 (Snakemake workflow generation)
- Next phase: Phase 3 — Snakemake workflow generation
- Next phase doc: `/home/dcl3nd/dev/agentic-workspace/library/docs/planning/projects/TRITON-SWMM_toolkit/features/per_sub_analysis_system_configs/3 snakemake workflow generation.md`
- Worktree: `/home/dcl3nd/dev/TRITON-SWMM_toolkit/.claude/worktrees/toolkit_05-15_0904_per-sub-analysis-system-configs-pwi`
...
1. Sidecar scratch doc:
   `/home/dcl3nd/dev/TRITON-SWMM_toolkit/.claude/worktrees/toolkit_05-15_0904_per-sub-analysis-system-configs-pwi/sidecars/sidecar_phase3_2026-05-15_1759.md`

2. Next phase doc:
   `/home/dcl3nd/dev/agentic-workspace/library/docs/planning/projects/TRITON-SWMM_toolkit/features/per_sub_analysis_system_configs/3 snakemake workflow generation.md`
...
- **Dependencies**: [1 config and instantiation](1%20config%20and%20instantiation.md), [2 compilation and preprocessing loop](2%20compilation%20and%20preprocessing%20loop.md)
...
The prior session halted at: `Phase 2 complete and committed; entering Phase 3 (Snakemake workflow generation)`
...
- **Worktree path**: `/home/dcl3nd/dev/TRITON-SWMM_toolkit/.claude/worktrees/toolkit_05-15_0904_per-sub-analysis-system-configs-pwi`
...
the Write tool directly — the sidecar lives under `/home/dcl3nd/dev/TRITON-SWMM_toolkit/.claude/worktrees/toolkit_05-15_0904_per-sub-analysis-system-configs-pwi/sidecars/`
...
`Checkpoint-match assertion: Sidecar: sidecar_phase3_2026-05-15_1759.md. Leave-off step (from
checkpoint): Phase 2 complete and committed; entering Phase 3 (Snakemake workflow generation). Next phase: Phase 3 — Snakemake workflow generation.
...
Do NOT read the main scratch doc. Do NOT compute hashes yourself. Invoke the script via `conda run` with the `-m` form:
    ...
    conda run -n agentic python -m scripts.generate.write_compaction_sidecar --mode verify \
      --main-scratch /home/dcl3nd/dev/agentic-workspace/library/_scratch/workspaces/TRITON-SWMM_toolkit_05-15_0904_per-sub-analysis-system-configs-pwi.md \
      --worktree /home/dcl3nd/dev/TRITON-SWMM_toolkit/.claude/worktrees/toolkit_05-15_0904_per-sub-analysis-system-configs-pwi
...
- **Sidecar-aware claude launcher (pre-cd routing affordance)** — Cluster-3 I4. Small shell tool (e.g., `claude-resume`) that scans every managed workspace's `.claude/worktrees/*/sidecars/` for eligible sidecars, presents a numbered list across workspaces, `cd`s to the chosen workspace's `repo_path` and execs `claude`. The only structural fix for cluster-3 Problem A (no pre-launch routing). Out of phase scope.
- **Cluster-1 I1 deferred — pre-clear assertion-first gate** — +175 words at `library/prompts/instructions/protocols/pwi post preflight compaction gate.md`. Deferred per cluster-1 BLUF in favor of the existing post-clear `check-compaction-gate-before-edit.sh` hook. Re-evaluate if the hook-detected omission rate becomes high enough to justify the prompt-token cost.
- **Empirical-testing recurrence question** — Phase 1's rebaseline self-commit introduces a new pattern (script-internal git commits with `--no-verify`). If generalizable across other script modes that produce uncommitted state as a side-effect of 'successful' exit, the agentic-workspace architecture's 'Gotchas' section may merit a broader entry describing the script-self-commit pattern, not just the rebaseline instance.
