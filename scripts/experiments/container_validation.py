...
config (SIF path + account + partitions) is resolved from the PRIVATE hhemt_projects estate
via ``_resolve_hpc_system_config`` ($HHEMT_PROJECTS / $HHEMT_HPC_SYSTEM_CONFIG / argv[2]) —
...
# Default desktop checkout of the PRIVATE deployment estate (hhemt_projects), computed
# from the user's home (portable; no hard-coded /home/<user>). Cluster runs set
# $HHEMT_PROJECTS to the compute-visible checkout (the resolver reads it).
_HHEMT_PROJECTS_DEFAULT = str(Path.home() / "dev" / "hhemt_projects")
...
def _resolve_hpc_system_config(cluster: str, override: str | None = None) -> Path:
    ...
    hhemt_projects estate (the versioned, git-pulled deployment-config home).

    Precedence: explicit ``override`` (argv[2]) > ``$HHEMT_HPC_SYSTEM_CONFIG`` env var >
    ``$HHEMT_PROJECTS/hpc/hpc_system_config_{cluster}.yaml`` (``$HHEMT_PROJECTS`` defaults to
    ...
    if override:
        ...
        projects = Path(os.environ.get("HHEMT_PROJECTS", _HHEMT_PROJECTS_DEFAULT)).expanduser()
        path = projects / "hpc" / f"hpc_system_config_{cluster}.yaml"
    if not path.is_file():
        raise FileNotFoundError(
            f"No hpc_system_config for cluster {cluster!r} at {path}.\n"
            f"  1. On the cluster, set $HHEMT_PROJECTS to your compute-visible hhemt_projects "
...
def build_case(
    ...
    *,
    ...
    + account) is resolved from the PRIVATE hhemt_projects estate (see
