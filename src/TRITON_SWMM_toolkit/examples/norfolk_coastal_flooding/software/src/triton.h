...
*/		
	...
	private:
		...
		T global_init_dt;	/**< Global time step size in case the domain is dry and the time step size is chosen dynamically*/
		...
		#if SWMM_TRITON
			SWMM_triton::swmm_triton swmm_model;
			int nbytes_swmm;	/**< SWMM water depth size in bytes */	
			T local_elapsedTime;
			long oldHour;
		#endif

#ifdef ACTIVE_GPU
		gpuStream_t streams;	/**< gpu stream */
...
*/		
	...
	void triton<T>::process_source_locations()
	{
		...
		if(arglist.num_sources > 0)
		{
			...
			for (int i = 0; i < num_sources; ++i)
			{
				...
				//check if the source has been already assigned from another source
				for (int j = 0; j < i; ++j){
					if((src_rows[j] == src_rows[i]) && (src_cols[j] == src_cols[i])){
						std::cerr << ERROR "Source " << i+1  << " has the same cell than source " << j+1  <<std::endl;
						exit(EXIT_FAILURE);
	...
	void triton<T>::create_device_vectors()
	{
		...
		#if SWMM_TRITON
			nbytes_swmm = (sizeof(T) * swmm_model.num_of_swmm_links);
		#endif
		...
		*device_bc_var1_arr, *device_bc_var2_arr, *device_max_value_h;
			...
			int *device_relative_swmm_node_index;
		#endif
		...
		*device_bc_start_index, *device_bc_nrows_vars;
		...
		#if SWMM_TRITON
			gpuMalloc((void**)&device_relative_swmm_node_index, nbytes_swmm_int);
		#endif
		...
		#if SWMM_TRITON
			device_vec_int.push_back(device_relative_swmm_node_index);
		#endif
	...
	void triton<T>::compute_new_state()
	{
		...
		if (num_of_extbc > 0 && num_extbc_cells > 0)
		{
			...
			if(swmm_model.num_of_swmm_links>0){
				...
				SWMM_triton::compute_swmm_triton_exchange <<<(swmm_model.num_of_swmm_links + THREAD_BLOCK - 1) / THREAD_BLOCK, THREAD_BLOCK, 0, streams >>> (swmm_model.num_of_swmm_links, cell_size, global_dt, device_vec[H], device_vec[QX], device_vec[QY], arglist.hextra, device_vec_int[SWMMP],device_vec[SWMM_LOSS], device_vec[SWMM_DIAMETER], device_vec[SWMM_MAXD], device_vec[SWMM_NEWD], device_vec[SWMM_Q]);
				...
				SWMM_triton::compute_swmm_triton_exchange(swmm_model.num_of_swmm_links, cell_size, global_dt, host_vec[H], host_vec[QX], host_vec[QY], arglist.hextra, host_vec_int[SWMMP], host_vec[SWMM_LOSS], host_vec[SWMM_DIAMETER], host_vec[SWMM_MAXD], host_vec[SWMM_NEWD], host_vec[SWMM_Q]);
			#endif

			}
	
			MPI_Gatherv(host_vec[SWMM_Q], swmm_model.num_of_swmm_links, MPI_DATA_TYPE, swmm_model.aux_global_exchange_q, swmm_model.counts, swmm_model.displs, MPI_DATA_TYPE, 0, MPI_COMM_WORLD);

			if(rank==0){
		   	swmm_model.local_to_global(swmm_model.aux_global_exchange_q, swmm_model.global_exchange_q, swmm_model.node_to_rank_dict);
				swmm_step(&local_elapsedTime, swmm_model.global_exchange_q, swmm_model.global_new_depth, global_dt);
				swmm_model.global_to_local(swmm_model.global_new_depth, swmm_model.aux_global_new_depth, swmm_model.node_to_rank_dict);
			}
			
			MPI_Scatterv(swmm_model.aux_global_new_depth, swmm_model.counts, swmm_model.displs, MPI_DATA_TYPE, host_vec[SWMM_NEWD], swmm_model.num_of_swmm_links, MPI_DATA_TYPE, 0, MPI_COMM_WORLD);
