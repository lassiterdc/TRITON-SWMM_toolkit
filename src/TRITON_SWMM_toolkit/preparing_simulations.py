...
def retrieve_sim_id_str(weather_event_indexers):
    sim_id_str = "_".join(f"{idx}.{val}" for idx, val in weather_event_indexers.items())
    return sim_id_str
...
def write_swmm_rainfall_dat_files(
    ...
    for gage in df_allrain:
        ...
        fname_raindat = "{}_grid-ind{}.dat".format(sim_id_str, gage)
        f_out_swmm_rainfall = sim_paths["dir_weather_datfiles"] / fname_raindat
...
def write_swmm_waterlevel_dat_files(
    storm_tide_units,
    ...
    f_out_swmm_wlevel = sim_paths["dir_weather_datfiles"] / fname_wleveldat
...
def run_swmm_hydro_model(
    ...
    if not (
        ...
        with Simulation(str(sim_paths[destination_sim_path_key])) as sim:
            sim.execute()
...
def create_external_boundary_condition_files(
    log,
    weather_timeseries,
    weather_event_indexers,
    weather_time_series_storm_tide_datavar,
    ...
    with open(sim_paths["extbc_tseries"], "w") as f:
        f.write(wlevel_first_line + wlevel_second_line)

    df_water_levels.to_csv(sim_paths["extbc_tseries"], mode="a", header=False)

    log["extbc_tseries"] = sim_paths["extbc_tseries"]
    # write external boundary condition location file
    rds_dem = rxr.open_rasterio(dem_processed)
    gdf_bc = gpd.read_file(variable_boundary_condition)
    ...
    BC_with_quotes = f'"{sim_paths["extbc_tseries"].name}"'
    str_line2 = "{},{},{},{},{},{}".format("1", x1, y1, x2, y2, BC_with_quotes)
    # write file
    fpath_extbc = sim_paths["extbc_loc"]
    with open(fpath_extbc, "w") as f:
        f.write(str_line1 + "\n")
        f.write(str_line2 + "\n")
    log["extbc_loc"] = sim_paths["extbc_loc"]
    log = update_logfile(log)
    return log
...
def write_hydrograph_files(
    ...
    with Output(hydro_outfile) as out:
        ...
        with open(sim_paths["hyg_timeseries"], "w") as f:
            f.write(inflow_first_line + inflow_second_line)
        df_node_inflow.to_csv(
            sim_paths["hyg_timeseries"], mode="a", index=False, header=False
        )
        log["hyg_timeseries"] = sim_paths["hyg_timeseries"]
        log = update_logfile(log)
        # write hydrograph location file
        str_first_line = "%X-Location,Y-Location"
        with open(sim_paths["hyg_locs"], "w") as f:
            ...
            for col in df_node_inflow.columns:
                ...
                y = col[1]
                f.write("{},{}\n".format(x, y))
        log["hyg_locs"] = sim_paths["hyg_locs"]
        log = update_logfile(log)
        ...
        df_xylocs = pd.read_csv(sim_paths["hyg_locs"], header=0, names=["x", "y"])
        ...
        df_hyg_loc = pd.read_csv(sim_paths["hyg_locs"])
        df_hyg_test = pd.read_csv(sim_paths["hyg_timeseries"], skiprows=2)
        if ((df_hyg_test.shape[1] - 1) - df_hyg_loc.shape[0]) != 0:
            ...
            print(f"{Path(sim_paths['hyg_locs']).parent}")
...
def update_hydraulics_model_to_have_1_inflow_node_per_DEM_gridcell(
    ...
    """
    df_node_locs, lst_outfalls = return_df_of_nodes_grouped_by_DEM_gridcell(
        log["inp_hydraulics"], dem_processed, verbose=verbose
    )
    ...
    # count = -1
    model = swmmio.Model(log["inp_hydraulics"])
    ...
    for grid_id, group in df_overlapping_nodes.groupby(["grid_id"]):
        ...
        upstream_nodes_to_drop = []  # initiate list of nodes to remove as inflow nodes
    ...
    with open(log["inp_hydraulics"], "r") as fp:
    ...
    with open(log["inp_hydraulics"], "w") as fp:
...
def generate_TRITON_SWMM_cfg(
    ...
    swmmmodel = swmmio.Model(str(sim_paths["inp_hydro"]))
    ...
    df_extbc_loc = pd.read_csv(log["extbc_loc"])
    num_ext_bc = len(df_extbc_loc)

    df_src_loc = pd.read_csv(sim_paths["hyg_locs"])
    ...
    mapping = dict(
        ...
        EXTBC_DIR=str(sim_paths["extbc_loc"].parent),
        EXTBC_FILE=log["extbc_loc"],
        SIM_DUR_S=sim_dur_s,
    ...
    create_from_template(
        triton_swmm_configuration_template, mapping, sim_paths["triton_swmm_cfg"]
    )
    log["triton_swmm_cfg"] = sim_paths["triton_swmm_cfg"]
    log = update_logfile(log)
    return log
